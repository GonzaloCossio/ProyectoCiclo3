...
def  inicio():
    ...
    if request.method=="POST":
        error=request.form["boton"]
        seleccion = get_db().execute(
            'SELECT * FROM producto WHERE id = ?', (error,)
        ).fetchone()
        return  render_template('producto.html',producto=seleccion,comentarios=comentarios)
    ...
    if request.method=="POST":
        ...
        for i in productos:
            if str(i[0]) in request.form:
                seleccion = get_db().execute(
                    'SELECT * FROM producto WHERE id = ?', (i[0],)
                    ).fetchone()
                db.execute(
                    'INSERT INTO comentario (usuario_id, producto_id, mensaje, calificacion) VALUES (?,?,?,?)',
                    (g.user[0],seleccion[0], comentario, calificacion)
                )
                db.commit()
                comentarios=db.execute(
                    'SELECT * FROM comentario'
...
def  wishlist():
    ...
    for i in productos:
        if str(i[0]) in request.form:
            ...
            return  render_template('wishlist.html',producto=seleccion, comentarios=comentarios)
...
def  comprar():
    ...
    for i in productos:
        if str(i[0]) in request.form:
            ...
            return  render_template('comprar.html',producto=seleccion, comentarios=comentarios)
    ...
    if request.method=="POST":
        ...
        if campo=="Nombre y Apellido":
            ...
            db.execute(
                    ...
                    (generate_password_hash(valor), usuario)
            )
            db.commit()
